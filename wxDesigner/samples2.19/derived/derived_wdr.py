# -*- coding: iso-8859-1 -*-

#-----------------------------------------------------------------------------
# Python source generated by wxDesigner from file: derived.wdr
# Do not modify this file, all changes will be lost!
#-----------------------------------------------------------------------------

# Include wxWindows' modules
import wx
import wx.grid

# Custom source
from derived2 import *

# Window functions

ID_TEXTCTRL1 = 10000
ID_TEXTCTRL2 = 10001
ID_LINE = 10002
wxID_OK = 5100
ID_TEST = 10003

def MyDialogFunc( parent, call_fit = True, set_sizer = True ):
    item0 = wx.BoxSizer( wx.VERTICAL )
    
    item2 = wx.StaticBox( parent, -1, "Special text controls" )
    item1 = wx.StaticBoxSizer( item2, wx.VERTICAL )
    
    item3 = MyTextCtrl( parent, ID_TEXTCTRL1, "", wx.DefaultPosition, [180,-1], 0 )
    item1.Add( item3, 0, wx.ALIGN_CENTER|wx.ALL, 10 )

    item4 = MyTextCtrl( parent, ID_TEXTCTRL2, "", wx.DefaultPosition, [180,-1], 0 )
    item1.Add( item4, 0, wx.ALIGN_CENTER|wx.ALL, 10 )

    item0.Add( item1, 0, wx.GROW|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 5 )

    item5 = wx.StaticLine( parent, ID_LINE, wx.DefaultPosition, [20,-1], wx.LI_HORIZONTAL )
    item0.Add( item5, 0, wx.GROW|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 5 )

    item6 = wx.BoxSizer( wx.HORIZONTAL )
    
    item7 = wx.Button( parent, wxID_OK, "OK", wx.DefaultPosition, wx.DefaultSize, 0 )
    item6.Add( item7, 0, wx.ALIGN_CENTER|wx.LEFT|wx.RIGHT|wx.BOTTOM, 5 )

    item8 = wx.Button( parent, ID_TEST, "Test", wx.DefaultPosition, wx.DefaultSize, 0 )
    item8.SetToolTip( wx.ToolTip("Copy text from first to second control") )
    item6.Add( item8, 0, wx.ALIGN_CENTER|wx.LEFT|wx.RIGHT|wx.BOTTOM, 5 )

    item0.Add( item6, 0, wx.ALIGN_CENTER|wx.ALL, 5 )

    if set_sizer == True:
        parent.SetSizer( item0 )
        if call_fit == True:
            item0.SetSizeHints( parent )
    
    return item0

# Menubar functions

# Toolbar functions

# Bitmap functions


# End of generated file
